...
function markUnsavedChanges(hasChanges = true) {
...
async function runSteps(messageContent, apiKey, model, callbacks = {}) {
      ...
      prompt = prompt.replace("{message_content}", messageContent);
        ...
        steps.forEach(prevStep => {
          ...
          if (stepResults[outputKey]) {
            prompt = prompt.replace(new RegExp(`{${outputKey}}`, 'g'), stepResults[outputKey]);
          }
          if (stepResults[summaryKey]) {
            prompt = prompt.replace(new RegExp(`{${summaryKey}}`, 'g'), stepResults[summaryKey]);
          }
        });
  ...
  /* Mark when the user is using a custom step configuration */
  function setCustomStepsMode(isCustom) {
    localStorage.setItem('custom_steps_mode', isCustom ? 'true' : 'false');
  }
  
  /* Check if we're in custom steps mode */
  function isCustomStepsMode() {
    return localStorage.getItem('custom_steps_mode') === 'true';
  }
