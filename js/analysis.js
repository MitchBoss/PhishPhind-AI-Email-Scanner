...
function displayResult(result) {
  ...
  // Create new tab
  const tabLink = $(`<a class="nav-link" id="tab-${result.stepName}" href="#${tabId}" role="tab">${result.menuName}</a>`);
  $("#resultTabs").append(tabLink);
  ...
  tabLink.on('click', function(e) {
    ...
    // Hide all tabs
    $('#resultTabs a').removeClass('active border-brand-purple text-brand-purple');
    $('#resultTabs a').addClass('text-gray-500');
    $('#resultTabsContent .tab-pane').hide();
    
    // Show active tab
    $(this).removeClass('text-gray-500');
    $(this).addClass('active border-brand-purple text-brand-purple');
    $(`#${tabId}`).show();
  });
  
  // If this is the first tab, show it
  if (result.stepNumber === 1) {
    tabLink.addClass('active border-brand-purple text-brand-purple').removeClass('text-gray-500');
    tabContent.show();
  }
