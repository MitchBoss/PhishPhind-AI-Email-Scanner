...
const StepService = (function() {
  ...
  function parseStepFile(content) {
    // More specific regex patterns to properly capture each section
    const menuNameMatch = content.match(/\[MENU_Name\](.*?)(?=\[STEP_PROMPT\])/s);
    const stepPromptMatch = content.match(/\[STEP_PROMPT\](.*?)(?=\[LLM_INSTRUCTIONS\])/s);
    
    // For LLM instructions, capture everything from the tag to the end of the file
    const llmInstructionsMatch = content.match(/\[LLM_INSTRUCTIONS\]([\s\S]*)/);
    
    return {
      menuName: menuNameMatch?.[1]?.trim() || '',
      stepPrompt: stepPromptMatch?.[1]?.trim() || '',
      llmInstructions: llmInstructionsMatch?.[1]?.trim() || ''
    };
  }
